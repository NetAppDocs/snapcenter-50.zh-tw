...
* 訊息佇列服務必須執行中。
* 如果您使用群組託管服務帳戶（GMSA）、則應該使用管理權限來設定GMSA。如需相關資訊，請參閱 link:task_configure_gMSA_on_windows_server_2012_or_later.html["在適用於Microsoft Exchange Server的Windows Server 2012或更新版本上設定群組託管服務帳戶"^]。
