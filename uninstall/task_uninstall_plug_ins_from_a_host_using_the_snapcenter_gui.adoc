...
.關於這項工作
您可以解除安裝個別外掛程式。 例如、您可能需要解除安裝 Microsoft SQL Server 的 SnapCenter 外掛程式、因為主機資源不足、您想要將該外掛程式移至功能更強大的主機。您也可以解除安裝整個外掛程式套件。例如、您可能需要解除安裝SnapCenter 適用於Linux的支援功能、其中包括SnapCenter 適用於Oracle資料庫的支援功能、SnapCenter 以及適用於UNIX的支援功能。
...
+
您也可以使用_uninstall-SmHostPackage_ Cmdlet和必要的參數、從多個主機解除安裝外掛程式。您可以執行_Get-Help命令name_來取得可搭配Cmdlet使用之參數及其說明的相關資訊。或者、您也可以參閱 https://docs.netapp.com/us-en/snapcenter-cmdlets-50/index.html["《軟件指令程式參考指南》SnapCenter"^]。
