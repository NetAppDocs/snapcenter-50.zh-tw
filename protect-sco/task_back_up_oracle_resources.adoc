...
.. 按一下 image:../media/add_policy_from_resourcegroup.gif["新增符號"] 「設定排程」欄位、為原則的所有排程類型設定驗證排程。+ 在「新增驗證排程 _ 原則名稱 _ 」對話方塊中、您可以執行下列步驟：
.. 選擇*備份後執行驗證*。
...
+
若要增加Java堆大小、請找出指令碼檔案_/opt/netapp/init_scripts/scvservice_。在該指令碼中、命令會 `do_start method` 啟動 SnapCenter VMware 外掛程式服務。請將該命令更新為 `Java -jar -Xmx8192M -Xms4096M`：。
